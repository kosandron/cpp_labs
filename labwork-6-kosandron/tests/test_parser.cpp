...
TEST(ParserTestSuite, MultiEasySectionTest) {
    std::string data = R"(
        [section1]
        key1 = 1
        key2 = true
    ...
    ASSERT_TRUE(root.Get("section1.1.key3").IsInt());
    ASSERT_EQ(root.Get("section1").Get("1").Get("key3").AsInt(), 3);
    ASSERT_EQ(root.Get("section1.1.key3").AsInt(), 3);
